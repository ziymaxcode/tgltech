...
export function ProductDetailsPage() {
  ...
  const alsoBuy = product.alsoBuy.map(id => STORE_PRODUCTS.find(p => p.id === id)).filter(Boolean);
  ...
  return (
    <div className="bg-[#fbfbfb] min-h-screen pb-32">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <Link to="/store" className="inline-flex items-center text-xs font-bold uppercase tracking-widest text-gray-400 hover:text-[#1d1d1f] transition-colors">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm flex flex-col md:flex-row">
          <div className="md:w-1/2 p-8 md:p-12 lg:p-16 flex items-center justify-center bg-[#fbfbfb] border-b md:border-b-0 md:border-r border-gray-100 relative">
            ...
            <img src={product.image} alt={product.name} className="max-h-96 object-contain mix-blend-multiply opacity-90" />
          </div>
          
          <div className="md:w-1/2 p-8 md:p-12 flex flex-col bg-white">
            <div className="flex justify-between items-start mb-4">
              <h1 className="text-4xl font-bold text-[#1d1d1f] leading-tight tracking-tight">{product.name}</h1>
            ...
            <div className="text-4xl font-bold text-[#1d1d1f] mb-8 tracking-tight">
              {product.price}
              <span className="text-xs font-bold uppercase tracking-widest text-gray-400 block mt-2">+ shipping</span>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16 space-y-16 group/sections">
        
        {/* Also Buy */}
        {alsoBuy.length > 0 && (
          <section>
            <h2 className="text-2xl font-bold text-[#1d1d1f] mb-6 flex items-center tracking-tight">
              <Sparkles className="w-6 h-6 mr-3 text-blue-600" /> Frequently Bought Together
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {alsoBuy.map((p: any) => (
                <div key={p.id} className="bg-white p-4 rounded-3xl border border-gray-100 hover:border-blue-600/30 transition-colors shadow-sm">
                  <img src={p.image} alt={p.name} className="h-32 w-full object-contain mix-blend-multiply opacity-90 mb-4" />
                  <Link to={`/store/${p.id}`} className="font-bold text-sm text-[#1d1d1f] line-clamp-2 hover:text-blue-600">
                    {p.name}
                  </Link>
                  <div className="mt-2 text-gray-500 font-medium text-sm">{p.price}</div>
                </div>
              ))}
            </div>
          </section>
        ...
        {projectsUsingThis.length > 0 && (
          <section>
            <h2 className="text-2xl font-bold text-[#1d1d1f] mb-6 flex items-center tracking-tight">
              <BookText className="w-6 h-6 mr-3 text-blue-600" /> Projects using {product.name}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {projectsUsingThis.map((proj: any) => (
                <Link key={proj.id} to={`/projects/${proj.id}`} className="bg-white rounded-3xl border border-gray-100 flex overflow-hidden hover:shadow-md transition-shadow">
                  <img src={proj.image} alt={proj.name} className="w-1/3 object-cover opacity-90" />
                  <div className="p-6 flex-1 flex flex-col justify-center">
                    <h3 className="font-bold text-lg mb-2 text-[#1d1d1f] leading-tight">{proj.name}</h3>
                    <p className="text-gray-500 text-sm mb-4 line-clamp-2 leading-relaxed">{proj.description}</p>
                    <span className="text-blue-600 font-bold text-[10px] uppercase tracking-widest">View Project →</span>
                  </div>
                </Link>
              ))}
            </div>
            ...
            {[1, 2, 3].map(i => (
              <div key={i} className="bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                <div className="aspect-video bg-[#1d1d1f] flex items-center justify-center relative group cursor-pointer">
                  <img src="https://images.unsplash.com/photo-1555255707-c07966088b7b?auto=format&fit=crop&q=80&w=600" className="opacity-40 group-hover:opacity-20 transition-opacity mix-blend-luminosity" />
                ...
                <div className="p-6">
                  <h4 className="font-bold text-[#1d1d1f] text-sm mb-2">How to wire {product.name} for beginners</h4>
                  <p className="text-[10px] uppercase tracking-widest font-bold text-gray-400">10 mins • Wiring Diagram included</p>
                </div>
              </div>
            ))}
          </div>
